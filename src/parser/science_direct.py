...
def new_science_direct_articles(url: str, after_title: str) -> tuple[List[Article], str]:
  articles: List[Article] = []
  res = feedparser.parse(url)
  last_title = ''
  for entry in res.entries:
    if entry.title == after_title:
      break
...
def extract_authors(summary: str) -> List[str] | None:
    ...
    return map(lambda author: author.strip(), authors)
